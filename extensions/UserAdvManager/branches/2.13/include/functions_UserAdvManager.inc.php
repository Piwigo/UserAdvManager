...
function SendMail2User($typemail, $id, $username, $password, $email, $confirm)
{
  ...
  /* ****************************************************************** */
  /* Delete this after new Piwigo release (bug in switch_lang function) */
  global $conf, $user;
  $save_user = $user; 
  /* ****************************************************************** */
...
/* And switch gallery to this language before using personalized and multilangual contents */
  ...
  if (isset($conf_nbc_UserAdvManager[19]) and $conf_nbc_UserAdvManager[19] <> '' and isset($conf_nbc_UserAdvManager[17]) and $conf_nbc_UserAdvManager[17] == 'true')
  {
    ...
    {
      $infos1 = l10n($conf_nbc_UserAdvManager[19])."\n\n";
    }
...
/* debugging              */
/* ********************** */
//$content = get_user_language_desc($conf_nbc_UserAdvManager[19])."\n\n";  
//MailLog($email,$subject,$content,$language);
/* ********************** */
...
/* Function called from UserAdvManager_admin.php to get the plugin version */
...
/* Funtion called in maintain.inc.php to clean obsolete files */
function clean_obsolete_files($obsolete_file_list)
{
  if (!file_exists(NBC_UAM_PATH.$obsolete_file_list))
  {
    return TRUE;
  }
  $obsolete = file(NBC_UAM_PATH.$obsolete_file_list);
  array_push($obsolete, $obsolete_file_list);
  return clean_obsolete_list($obsolete);
}

function clean_obsolete_list($file_list = array(), &$errors = array())
{
  if (!function_exists('unlink'))
  {
      // No unlink available...
      array_push($errors, l10n('uam_no_unlink'));
      return FALSE;
  }
  $success = TRUE;
  foreach ($file_list as $file)
  {
    $file = NBC_UAM_PATH . $file;
    if (file_exists($file))
    {
  ...
  if (!$success)
  {
      array_push($errors, l10n('uam_unlink_errors'));
  }
  return $success;
}
?>