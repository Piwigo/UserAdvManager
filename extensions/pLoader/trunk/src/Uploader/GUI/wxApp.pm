...
sub OnInit {
    ...
    my $not_exit = $self->Login();
    # user pressed OK
    if($not_exit){
    	if( !$self->use_offline ){
            while( $not_exit and !$self->_is_connected ){
            	$not_exit = $self->Login();
            	last if $self->use_offline;
            }
    	}
...
sub init_locale {
    ...
    if(!$locale->AddCatalog( 'pLoader.mo' )){
        Wx::LogMessage gettext("Cannot find translation catalog files. Use default language");
    }
...
sub _init_imagelist {
    ...
    if( -e $self->storable_file ){
        ...
        if($@){
            Wx::LogMessage(
                gettext("An error has occured. Can not read %s\n%s"),
                $self->storable_file,
                $@
            );
            $use_new_imagelist = 1 ;
        }
        # should have a valid imagelist
        else{
            $use_new_imagelist = 1 unless $self->imagelist_version eq $stored_imagelist->{imagelist_version};
            if($use_new_imagelist){
            	Wx::LogMessage(gettext("pLoader has to reset image cache."));
            }
        }
    }
    else{
    ...
    $self->imagelist->RescaleCallback(
        sub { $self->RescaleImage(@_) }
    );
...
sub _default_imagelist_params {
    ...
    my $params = {
        ...
        site_resized_dir => $self->resized_dir,
      	...
      	th_quality       => 90,
        auto_rotate      => 1,
        remove_uploaded_from_selection => 1,
        interlace        => 'Line',
        create_resized   => 1,
        use_exif_preview => 1,
      	prefix           => 'TN',
        ...
        default_photo_name => gettext('File name'),
        default_name_prefix => gettext('Photo'),
        upload_rejects   =>  [],
        image_sums       => {},
        sums             => [],
        version          => $self->version,
        imagelist_version => $self->imagelist_version,
        RescaleCallback  => sub { $self->RescaleImage(@_) },
        ...
        Uploader::GUI::wxLoginDlg->new(
            {
                ...
                use_offline   => sub { $self->use_offline(@_) },	
            }
        )
    ) unless $self->login_dlg;

    my $icon = Wx::Icon->new();
    $icon->LoadFile('../res/favicon.ico', wxBITMAP_TYPE_ICO);
    $self->login_dlg->SetIcon($icon);	

    
    my $rval = $self->login_dlg->ShowModal();
    $self->login_dlg->Show(0);
...
sub _reload_thumb_images {
    ...
    my $sums = $self->imagelist->sums;

    map {
    	my $image = $self->imagelist->image_sums->{$_};
...
sub RescaleImage {
    ...
    if(!$image->SaveFile(
    	...
    	Wx::LogMessage(
    	    gettext("An error has occured. Can not save file %s"),
    	    $image_file_out,
    	)
    };
}
...
sub ResizeImage {
    my ( $self, $image_file, $image_file_out, $type, $width, $height, $quality ) = @_;
...
# some labels differ with branding ( piwigo.com or piwigo.org )
sub _init_branding {
    my ( $self ) =@_;
    
    if( $self->site_url =~ /\.piwigo\.com/ ){
        $self->branding(
            {
                category  => gettext("album"),	
                Category  => gettext("Album"),	
                categories => gettext("albums"),	
                Categories => gettext("Albums"),
                'Add new category' => gettext("Add new album"),	
            }
        );
    }
    else{
        $self->branding(
            {
                ...
                Categories => gettext("Categories"),	
                'Add new category' => gettext("Add new category"),	
            }
        );
    }	
}
...
sub SaveConfig {
   ...
   map {
       $config->WriteInt( $_, $params->{$_} )
   } keys %$params;

   $config->Write( 'Perspective', $params->{Perspective} )
...
sub _init_frame {
    ...
    my $prev_w = $config->ReadInt( "PreviewWidth", 400 );
    my $prev_h = $config->ReadInt( "PreviewHeight", 300 );
    my $perspective = $config->Read( "Perspective" );
    
    #printf("Config x %s, y %s, w %s, h %s, prev_w %s, prev_h %s\n", $x, $y, $w, $h, $prev_w, $prev_h);
    
    my $url = $self->site_url;
    
    if($self->use_offline){
    	$url = gettext("Work Offline");
    }
    ...
    my ( $frame ) = Uploader::GUI::wxFrameAUI->new( 
        {
            ...
            width     => $w,
            height    => $h,
            title     => sprintf("pLoader - Piwigo uploader %s - [%s]", $self->version, $url),
            pwg       => $self->pwg,
